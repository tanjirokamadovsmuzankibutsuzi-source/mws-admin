      ...
      <aside className="hidden md:flex w-56 shrink-0 border-r border-[#1E1E2A] flex-col bg-[#111118] fixed inset-y-0 left-0 z-30">
        <div className="px-5 py-5 border-b border-[#1E1E2A]">
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-bold text-white text-sm">MWS Admin</p><p className="text-[#6B6B80] text-xs">v2.1</p></div>
          </div>
        </div>
...
// ── AUDIO PRESETS ─────────────────────────────────────────────────────────────
const AUDIO_PRESETS = [
  { label: 'HIN+ENG', value: '-audLang=hin,eng' },
  { label: 'HIN+ENG+TAM', value: '-audLang=hin,eng,tam' },
  { label: 'HIN+ENG+TAM+TEL', value: '-audLang=hin,eng,tam,tel' },
  { label: 'MULTI', value: '-audLang=hin,eng,tam,tel,kan,mal' },
  { label: 'ENG ONLY', value: '-audLang=eng' },
]
...
function PostTab({ toast, role: _r, username: _u }: { toast:(m:string,t?:Toast['type'])=>void; role:string; username:string }) {
  ...
  function insertAtCursor(insert: string) {
    ...
    const newText = text.slice(0, start) + (start > 0 && text[start-1] !== '\n' ? '\n' : '') + insert + '\n' + text.slice(end)
    setText(newText)
    setTimeout(() => { el.selectionStart = el.selectionEnd = start + insert.length + 1 }, 0)
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto">
      <div className="mb-5"><h1 className="text-xl md:text-2xl font-bold text-white">Post Files</h1><p className="text-[#6B6B80] text-sm mt-1">Add to DB & broadcast to Telegram</p></div>

      {/* Quick Insert Shortcuts */}
      <div className="glass rounded-2xl p-4 mb-3">
        <p className="text-xs font-semibold text-[#6B6B80] uppercase tracking-widest mb-3">Quick Insert</p>
        <div className="space-y-2">
          {/* Audio presets */}
          <div>
            <p className="text-xs text-[#6B6B80] mb-1.5">🎵 Audio Presets</p>
            <div className="flex flex-wrap gap-1.5">
              {AUDIO_PRESETS.map(p => (
                <button key={p.value} onClick={() => insertAtCursor(p.value)}
                  className="text-xs bg-white/5 hover:bg-[#E50914]/10 hover:text-[#E50914] hover:border-[#E50914]/30 border border-[#1E1E2A] text-[#6B6B80] px-3 py-1.5 rounded-lg transition-all font-mono">
                  {p.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <p className="text-xs text-[#6B6B80] mb-1.5">🔘 Custom Button</p>
            <div className="flex gap-1.5">
              <button onClick={() => insertAtCursor('-dlbtn1<Server 1>=https://your-link-here')}
                className="text-xs bg-white/5 hover:bg-[#E50914]/10 hover:text-[#E50914] hover:border-[#E50914]/30 border border-[#1E1E2A] text-[#6B6B80] px-3 py-1.5 rounded-lg transition-all font-mono">
                + Button 1
              </button>
              <button onClick={() => insertAtCursor('-dlbtn2<Server 2>=https://your-link-here')}
                className="text-xs bg-white/5 hover:bg-[#E50914]/10 hover:text-[#E50914] hover:border-[#E50914]/30 border border-[#1E1E2A] text-[#6B6B80] px-3 py-1.5 rounded-lg transition-all font-mono">
                + Button 2
              </button>
              <button onClick={() => insertAtCursor('-dlbtn3<Server 3>=https://your-link-here')}
                className="text-xs bg-white/5 hover:bg-[#E50914]/10 hover:text-[#E50914] hover:border-[#E50914]/30 border border-[#1E1E2A] text-[#6B6B80] px-3 py-1.5 rounded-lg transition-all font-mono">
                + Button 3
              </button>
            </div>
      ...
      <div className="glass rounded-2xl p-4 mb-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setText('')} className="text-xs text-[#6B6B80] hover:text-red-400 transition-all">Clear</button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {(['telegram','json','web'] as const).map(mode => (
          <button key={mode} onClick={() => handlePreview(mode)} disabled={previewLoading}
            ...
            <Icon.eye />{mode==='telegram'?'📱 TG':mode==='json'?'📄 JSON':'🌐 Web'}
          </button>
        ))}
      </div>
      ...
      <div className="mt-4 glass rounded-2xl p-4">
        ...
        <div className="grid grid-cols-1 gap-2 text-xs font-mono">
          {([
            ['With IMDB','tt1234567\nFilename [25GB]\nhttps://hub-link'],
            ['Custom Buttons','Movie [4GB]\nhttps://link\n-dlbtn1<Server1>=https://url\n-dlbtn2<Server2>=https://url'],
            ['Bulk Audio','Movie1 [2GB]\nhttps://link1\nMovie2 [3GB]\nhttps://link2\n-audLang=hin,eng'],
            ['Tags','Movie [4GB] #Exclusive #Trending\nhttps://link'],
...
function DbTab({ toast, role, username }: { toast:(m:string,t?:Toast['type'])=>void; role:string; username:string }) {
  ...
  const [customBtns, setCustomBtns] = useState<{text:string;url:string}[]>([])
  ...
  function startEdit(item: DbItem, field: string) {
    ...
    else if (field==='custom_buttons') {
      setCustomBtns(Array.isArray(val) ? (val as {text:string;url:string}[]) : [])
      setEditValue('')
    }
  ...
  async function saveEdit() {
    ...
    setSaving(true)
    try {
      const value = editField === 'custom_buttons' ? customBtns : editValue
      const res = await fetch('/api/db', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ uid:editItem.uid, field:editField, value }) })
      const data = await res.json()
      if (res.ok) {
        if (editField === 'imdb_id') toast('✅ Saved & TMDB data fetched!', 'success')
        else toast('Saved!','success')
        setEditItem(null); load(page)
      }
      else toast(data.error,'error')
  ...
  const EDITABLE_FIELDS = [
    ...
    {key:'hub',label:'☁️ Hub URL'},{key:'gdf',label:'⚡ GDF URL'},
    {key:'backdrop',label:'🖼 Backdrop URL'},{key:'genre',label:'🎭 Genres'},
    {key:'tags',label:'🏷 Tags'},{key:'rating',label:'⭐ Rating'},
    {key:'type',label:'📺 Type'},{key:'imdb_id',label:'🔍 IMDB ID (auto-fetch)'},{key:'tmdb_id',label:'🎥 TMDB ID'},
    {key:'custom_buttons',label:'🔘 Custom Buttons'},
  ]
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 bg-[#E50914]/10 border border-[#E50914]/30 rounded-xl px-4 py-2.5 mb-3 animate-fade-in">
          ...
          <button onClick={bulkDelete} disabled={bulkDeleting}
            ...
            <Icon.trash />{bulkDeleting?'Deleting...':'Delete Selected'}
          </button>
          <button onClick={() => setSelected(new Set())} className="text-xs text-[#6B6B80] hover:text-white transition-all">✕</button>
        </div>
      )}
      ...
      <div className="space-y-2">
        {items.map(item => (
          <div key={item.uid} className={`glass rounded-xl p-3 transition-all ${selected.has(item.uid)?'border-[#E50914]/30':''}`}>
            <div className="flex items-start gap-2">
              {canEdit(item) && (
                <button onClick={() => toggleSelect(item.uid)} className="shrink-0 mt-1.5">
              ...
              {item.backdrop && <img src={item.backdrop} alt="" className="w-12 h-8 object-cover rounded-lg shrink-0 hidden sm:block" />}
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1 overflow-hidden">
                    {/* Fixed: name stays in box */}
                    <p className="text-white text-sm font-medium break-words leading-tight line-clamp-2">{item.name}</p>
                    {item.title && <p className="text-[#6B6B80] text-xs mt-0.5 truncate">🎬 {item.title}{item.year ? ` (${item.year})` : ''}</p>}
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                      <span className="text-[#6B6B80] text-xs">💾 {item.size}</span>
                      <span className="text-[#6B6B80] text-xs truncate">🎵 {item.audio||item.langs||'—'}</span>
                      ...
                      {item.custom_buttons?.length ? <span className="text-amber-400 text-xs">🔘×{item.custom_buttons.length}</span> : null}
                    </div>
                  ...
                  {canEdit(item) && (
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <button onClick={() => { setEditItem(item); setEditField(''); setEditValue('') }}
                        className="flex items-center gap-1 text-xs text-[#6B6B80] hover:text-white bg-white/5 hover:bg-white/10 px-2.5 py-1.5 rounded-lg transition-all whitespace-nowrap">
                        <Icon.edit /> Edit
                      </button>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end md:items-center justify-center z-50 p-4">
          <div className="glass-strong rounded-2xl p-5 w-full max-w-md max-h-[85vh] overflow-auto animate-slide-up">
            ...
            <p className="text-white text-xs font-medium mb-1 truncate">{editItem.name}</p>
            ...
            {!editField ? (
              ...
              <div>
                ...
                {editField === 'imdb_id' && (
                  <p className="text-xs text-blue-400 mb-2">💡 Saving will auto-fetch title, backdrop, genres from TMDB</p>
                )}
                {(editField==='hub'||editField==='gdf') && (
                  <p className="text-xs text-amber-400 mb-2">⚠️ Enter raw URL (auto-encoded)</p>
                )}

                {editField === 'custom_buttons' ? (
                  <div className="space-y-2 mb-3">
                    {customBtns.map((btn, i) => (
                      <div key={i} className="flex gap-2 items-center">
                        <div className="flex-1 space-y-1">
                          <input value={btn.text} onChange={e => setCustomBtns(prev => prev.map((b,j) => j===i?{...b,text:e.target.value}:b))}
                            placeholder="Button label" className="w-full bg-[#0A0A0F] border border-[#1E1E2A] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#E50914]/40" />
                          <input value={btn.url} onChange={e => setCustomBtns(prev => prev.map((b,j) => j===i?{...b,url:e.target.value}:b))}
                            placeholder="https://url" className="w-full bg-[#0A0A0F] border border-[#1E1E2A] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#E50914]/40" />
                        </div>
                        <button onClick={() => setCustomBtns(prev => prev.filter((_,j) => j!==i))} className="shrink-0 text-red-400 hover:text-red-300 p-1"><Icon.trash /></button>
                      </div>
                    ))}
                    <button onClick={() => setCustomBtns(prev => [...prev, {text:'',url:''}])}
                      className="flex items-center gap-1.5 text-xs text-[#6B6B80] hover:text-white border border-dashed border-[#1E1E2A] hover:border-white/20 px-3 py-2 rounded-lg transition-all w-full justify-center">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={saveEdit} disabled={saving}
                    className="flex-1 py-2.5 rounded-xl bg-[#E50914] text-white text-sm font-semibold hover:bg-red-700 transition-all disabled:opacity-50">
                    {saving ? (editField==='imdb_id' ? 'Fetching...' : 'Saving...') : 'Save'}
                  </button>
                </div>
              </div>
            )}
...
function LinkTab({ toast }: { toast:(m:string,t?:Toast['type'])=>void }) {
  ...
  async function addToSelected(item: {uid:string;name:string;size:string;audio:string}) {
    ...
    setSelected(prev => [...prev, item]); setSuggestions([])
    try {
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto">
      <div className="mb-6"><h1 className="text-xl md:text-2xl font-bold text-white">Get Links</h1><p className="text-[#6B6B80] text-sm mt-1">Search, select multiple & copy links</p></div>
      ...
      {selected.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-[#6B6B80] font-semibold uppercase tracking-widest">{selected.length} selected</p>
            ...
            return (
              <div key={item.uid} className="glass rounded-xl p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button onClick={() => removeSelected(item.uid)} className="shrink-0 text-[#6B6B80] hover:text-red-400 text-sm">×</button>
                </div>
                {!links ? (
                  <p className="text-[#6B6B80] text-xs">Loading...</p>
                ) : (
                  <div className="space-y-1.5">
                    ...
                    {!links.hub && !links.gdf && <p className="text-[#6B6B80] text-xs">No links for selected source</p>}
                  </div>
                )}
              </div>
            )
    ...
    <div className="p-4 md:p-8">
      ...
      {section==='migrate' && role==='owner' && (
        <div className="glass rounded-2xl p-5 space-y-4">
          <div><h3 className="text-white font-bold mb-1">🔑 UID Migration Tool</h3><p className="text-[#6B6B80] text-sm">Run once to add UIDs to old entries.</p></div>
          {migrateInfo && (
...
function BinTab({ toast, role, username }: { toast:(m:string,t?:Toast['type'])=>void; role:string; username:string }) {
  ...
  async function permDelete(uid: string) {
    ...
    if (res.ok) { toast('Deleted permanently','success'); load() }
    else { const d = await res.json(); toast(d.error,'error') }
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-5 flex items-center justify-between">
        ...
        <button onClick={load} className="text-xs bg-white/5 hover:bg-white/10 text-[#6B6B80] hover:text-white px-3 py-2 rounded-xl border border-[#1E1E2A] transition-all">🔄 Refresh</button>
      </div>
          ...
          {items.map(item => (
            <div key={item.uid} className="glass rounded-xl p-3 md:p-4">
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-white text-sm font-medium break-words line-clamp-2">{item.name}</p>
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                    <span className="text-[#6B6B80] text-xs">💾 {item.size}</span>
                ...
                {(role==='owner'||item.uploader_id===username) && (
                  <div className="flex flex-col gap-1.5 shrink-0">
                    <button onClick={() => restore(item.uid)} className="flex items-center gap-1 text-xs text-green-400 bg-green-500/10 hover:bg-green-500/20 px-2.5 py-1.5 rounded-lg transition-all whitespace-nowrap"><Icon.restore /> Restore</button>
                    {role==='owner' && <button onClick={() => permDelete(item.uid)} className="flex items-center gap-1 text-xs text-red-400 bg-red-500/10 hover:bg-red-500/20 px-2.5 py-1.5 rounded-lg transition-all whitespace-nowrap"><Icon.trash /> Delete</button>}
                  </div>
                )}
              </div>
            </div>
