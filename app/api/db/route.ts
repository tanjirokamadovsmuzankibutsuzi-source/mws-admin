...
import { fetchTmdbFromImdb } from '@/lib/tmdb'
...
export async function GET(req: NextRequest) {
  ...
  if (q) {
    ...
    data = data.filter(item =>
      ...
      (item.audio || (item as unknown as Record<string,string>).langs || '').toLowerCase().includes(lower) ||
...
export async function PATCH(req: NextRequest) {
  ...
  if (field === 'hub' || field === 'gdf') {
    ...
    // value should be array of {text, url} objects
    item.custom_buttons = Array.isArray(value) ? value.map((btn: {text:string;url:string}) => ({
      text: btn.text,
      url: btn.url.startsWith('aHR') ? btn.url : encodeLink(btn.url) // don't double-encode
    })) : []
  } else if (field === 'imdb_id') {
    // Save IMDB ID then auto-fetch TMDB data
    itemAny[field] = value
    if (value && value.startsWith('tt')) {
      const meta = await fetchTmdbFromImdb(value)
      if (meta) {
        if (meta.title) item.title = meta.title
        if (meta.year) item.year = meta.year
        if (meta.backdrop) item.backdrop = meta.backdrop
        if (meta.genres) item.genre = meta.genres.split(' | ')
...
export async function DELETE(req: NextRequest) {
  ...
  if (!deleted.length) return NextResponse.json({ error: 'No entries deleted' }, { status: 403 })

  // Sequential push to avoid SHA conflict
  ...
  if (!p2) return NextResponse.json({ error: 'Bin push failed' }, { status: 500 })
